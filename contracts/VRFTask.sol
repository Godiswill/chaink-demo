...
contract VRFTask is VRFConsumerBaseV2 {
     ...
     * 测试网地址：https://goerli.etherscan.io/address/0xbfb9ed5716032449ab170d3722c8b3ea33f21649
     *  */
    function fulfillRandomWords(
        ...
        //在此添加 solidity 代码
        s_randomWords = new uint256[](NUM_WORDS);
        for (uint256 i = 0; i < NUM_WORDS; i++) {
            s_randomWords[i] = i + 1;
        }
        for (uint256 i = 0; i < _randomWords.length; i++) {
            uint256 tmp = _randomWords[i] % NUM_WORDS;
            if (tmp == i) continue;
            uint256 aux = s_randomWords[i];
            s_randomWords[i] = s_randomWords[tmp];
            s_randomWords[tmp] = aux;
        }
